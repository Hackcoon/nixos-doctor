...

{
    ...
    echo "### flake"; git -C /etc/nixos status --short 2>/dev/null; stat -c 'lock-mtime: %y' /etc/nixos/flake.lock 2>/dev/null
    echo "### desktop"
    command -v mango >/dev/null 2>&1 && mango -p -c ~/.config/mango/config.conf 2>&1
    command -v dms >/dev/null 2>&1 && systemctl --user is-active dms.service mango-session.target 2>/dev/null
...
{ lspci; echo ---; nvidia-smi -L 2>/dev/null; sensors 2>/dev/null; } >"$WORK/hardware.txt" 2>&1
